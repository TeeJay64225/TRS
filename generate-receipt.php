...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(100, 30, 'TRS RECEIPT', 0, 0, 'L'); // Left aligned TRS
...
if (isset($_GET['receipt_id'])) {
    ...
    $stmt->bind_param('i', $receipt_id);  // Use prepared statements to avoid SQL injection
    $stmt->execute();
    ...
    // Proceed with further logic like generating PDFs, displaying items, etc.
    ...
    $pdf->Cell(0, 10, "RECEIPT NO:# " . $receipt['receipt_number'], 0, 1, 'R');
    $pdf->Cell(0, 10, "RECEIPT DATE: " . date('d M Y', strtotime($receipt['date'])), 0, 1, 'R');
    ...
    $pdf->SetXY(20, 85);
    $pdf->Cell(100, 10, 'RECEIPT TO:', 0, 1);
